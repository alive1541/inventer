...
import getLocalComponnetByURL from './getLocalComponnetByURL'
import React, { useContext, useEffect, useRef, useState } from "react"
import RenderContext from "./RenderContext"
import { Bridge, RenderOptions, Node, Topic } from '@inventer/meta'
...
function InnerRender({ node, C, inheritProps }: NodeRenderProps & { C: React.ElementType }) {
	...
	return (
		<Draggable
			...
			onDrag={e => {
				if (node.isDraggable()) {
					console.log('Draggable, UIEvents.EvtNodeSyncMoving ')
...
const NodeRender = ({ node, inheritProps }: NodeRenderProps) => {
	if (node.meta.url) {
		const localComponent = getLocalComponnetByURL(node.meta.url)
		console.log('node.meta.url', node.meta.url)
		if (localComponent) {
