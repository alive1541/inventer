...
class InstanceData extends Emiter<Topic> {
	...
	public updateInstanceData(key: string, updater: (value: any) => any) {
		this.data = this.data.updateIn([key], updater)
	}
	updateInstanceByPath(path: Array<string>, value: any) {
		this.data = PropsMeta.setPropValue(path, this.data, value)
